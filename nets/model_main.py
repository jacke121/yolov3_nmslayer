...
from nets.layers import NMSLayer
...
class ModelMain(nn.Module):
    def __init__(self, config, is_training=True):
        ...
        if not self.training:
            self.nms= NMSLayer()
        for i in range(3):
            yololayer = YOLOLayer(config["batch_size"], i, config["yolo"]["anchors"][i],
    ...
    def forward(self, x, targets=None):
        ...
        for i in range(3):
            _loss_item= self.yolo_losses[i](outputs[i], targets)
            if targets is None:
                detections.append(_loss_item)
            else:
                for j, l in enumerate(_loss_item):
                    losses[j] += l
        if targets is None:
            aaa=torch.cat(detections,1)
            start=time.time()
            result = self.nms(aaa)
